...
/**
 * The LinaShortcuts component is responsible for rendering a list of shortcuts based on the navigation and shortcuts props.
 * It uses various MUI components to render the list items and search input.
 * The component is memoized to prevent unnecessary re-renders.
 */
function LinaShortcuts(props) {
					...
					<div className={clsx('flex flex-1', variant === 'vertical' && 'flex-col')}>
								...
								_item && (
									...
									>
										...
										>
											...
											>
												{_item.icon ? (
													<LinaSvgIcon>{_item.icon}</LinaSvgIcon>
												) : (
						...
						>
							<IconButton
								...
								size="large"
							>
								<LinaSvgIcon sx={{ color: amber[600] }}>heroicons-solid:star</LinaSvgIcon>
							</IconButton>
						</Tooltip>
					</div>
				);
		...
		>
			<MenuItem key={item.id}>
				<ListItemIcon className="min-w-40">
					{item.icon ? (
						<LinaSvgIcon>{item.icon}</LinaSvgIcon>
					) : (
...
export default memo(LinaShortcuts);
