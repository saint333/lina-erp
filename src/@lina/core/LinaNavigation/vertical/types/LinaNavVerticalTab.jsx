...
/**
 *  The `LinaNavVerticalTab` component renders vertical navigation item with an adaptable
 *  layout to be used within the `LinaNavigation`. It only supports the `type`s of 'item',
 *  'selection' and 'divider'
 * */
function LinaNavVerticalTab(props) {
			...
			<Root sx={item.sx}>
				<ListItemButton
					...
					className={clsx(
						...
						'lina-list-item flex flex-col items-center justify-center p-12'
					)}
					...
					{dense ? (
						...
						>
							<div className="relative flex h-32 min-h-32 w-32 items-center justify-center">
								{item.icon ? (
									<LinaSvgIcon
										className={clsx('lina-list-item-icon', item.iconClass)}
										color="action"
									>
										{item.icon}
									</LinaSvgIcon>
						...
						<>
							...
							<ListItemText
								className="lina-list-item-text w-full grow-0"
								primary={item.title}
								classes={{
									primary:
										'text-12 font-medium lina-list-item-text-primary truncate text-center truncate'
								}}
							/>
						</>
					)}
...
const NavVerticalTab = LinaNavVerticalTab;
