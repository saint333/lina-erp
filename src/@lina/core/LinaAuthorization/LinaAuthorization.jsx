...
} from '@lina/core/LinaAuthorization/sessionRedirectUrl';
...
/**
 * LinaAuthorization is a higher-order component that wraps its child component which handles the authorization logic of the app.
 * It checks the provided Auth property from LinaRouteItemType (auth property) against the current logged-in user role.
 */
class LinaAuthorization extends Component {
	...
	static getDerivedStateFromProps(props, state) {
		...
		const { route } = matched;
		const userHasPermission = LinaUtils.hasPermission(route.auth, userRole);
	...
	render() {
		...
		return accessGranted ? children : <LinaLoading />;
	}
}
LinaAuthorization.contextType = AppContext;
export default withRouter(LinaAuthorization);
