...
const Root = styled('div')(({ theme }) => ({
	'& .LinaSearch-container': {
		position: 'relative'
	},
	'& .LinaSearch-suggestionsContainerOpen': {
		position: 'absolute',
		zIndex: 1,
	...
	'& .LinaSearch-suggestion': {
		display: 'block'
	},
	'& .LinaSearch-suggestionsList': {
		margin: 0,
		padding: 0,
		listStyleType: 'none'
	},
			...
			{variant === 'basic' ? (
				...
				<>
					<TextField
						...
						InputProps={{
							...
							classes: {
								input: 'LinaSearch-input py-0 px-16 h-40 md:h-48 ltr:pr-48 rtl:pl-48',
				...
				<TextField
					...
					InputProps={{
						...
						classes: {
							input: 'LinaSearch-input py-0 px-16 h-48 md:h-64'
						}
					}}
					variant="standard"
					{...other}
	...
	return (
		...
		>
			<ListItemIcon className="min-w-40">
				{suggestion.icon ? (
					<LinaSvgIcon>{suggestion.icon}</LinaSvgIcon>
				) : (
...
/**
 * LinaSearch component
 */
function LinaSearch(props) {
	const {
		navigation = [],
		className,
		variant = 'full',
		...
		trigger = (
			<IconButton
				...
				size="large"
			>
				<LinaSvgIcon>heroicons-outline:search</LinaSvgIcon>
			</IconButton>
		)
	} = props;
				...
				>
					<Autosuggest
						...
						theme={{
							...
							suggestionsList: 'LinaSearch-suggestionsList',
							suggestion: 'LinaSearch-suggestion'
						}}
...
export default memo(LinaSearch);
